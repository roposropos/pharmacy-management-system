	...
	SensitiveDataProtector sensitiveDataProtector)
{
	...
	private void Add(Klient klient)
	{
		...
		var pesel = klient.Pesel.Trim();
		klient.Pesel = pesel;
		command.Parameters.Add(new OdbcParameter("@Imie", osoba.Imie));
		command.Parameters.Add(new OdbcParameter("@Nazwisko", osoba.Nazwisko));
		command.Parameters.Add(new OdbcParameter("@Pesel", sensitiveDataProtector.Protect(pesel)));
		command.Parameters.Add(new OdbcParameter("@PeselHash", sensitiveDataProtector.Hash(pesel)));
	...
	private void Update(Klient klient)
	{
		...
		command.CommandText = """
		                      ...
		                      UPDATE apteka.klienci SET pesel = ?, pesel_hash = ? WHERE id_klienta = ?;
		                      """;
